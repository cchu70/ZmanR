"""Run Palantir pseudotime on scumi metacells using hvg_smooth_ctet gene set."""
...
OUT_PLOT   = "/home/unix/cchu/projects/ZmanR/pqe/results/pseudotime_scumi_hvg_smooth_ctet/palantir_pseudotime.png"

igg_df   = pd.read_csv(f"/home/unix/cchu/projects/ZmanR/pqe/results/06/ctet_mos/spearman/scumi_igg_hvg_smooth_ctet_spearman.csv",   index_col=0)
atrem_df = pd.read_csv(f"/home/unix/cchu/projects/ZmanR/pqe/results/06/ctet_mos/spearman/scumi_atrem_hvg_smooth_ctet_spearman.csv", index_col=0)
genes = list(set(igg_df.index[igg_df["p_value"] < 0.05]) & set(atrem_df.index[atrem_df["p_value"] < 0.05]))
print(f"scumi hvg smooth_ctet gene intersection: {len(genes)} genes")
...
ax.set_title("Palantir pseudotime (scumi hvg_smooth_ctet)"); ax.set_xlabel("sc_x"); ax.set_ylabel("sc_y")
