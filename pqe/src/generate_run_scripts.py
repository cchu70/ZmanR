...
def write_palantir_py(label, geneset_key, adata_path, out_dir, gene_loader_py):
...
for label, adata_path in ADATA_PATHS.items():
    for geneset_key, (_, r_loader_fn, py_loader_fn) in GENE_SETS.items():
        ...
        write_dpt_r(label, geneset_key, adata_path, out_dir, r_code)
        write_monocle2_r(label, geneset_key, adata_path, out_dir, r_code)
        write_redpath_r(label, geneset_key, adata_path, out_dir, r_code)
        write_scorpius_r(label, geneset_key, adata_path, out_dir, r_code)
        write_palantir_py(label, geneset_key, adata_path, out_dir, py_code)
