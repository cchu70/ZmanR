"""Run Palantir pseudotime on mcumi metacells using full_ctet gene set."""
...
OUT_PLOT   = "/home/unix/cchu/projects/ZmanR/pqe/results/pseudotime_mcumi_full_ctet/palantir_pseudotime.png"

igg_df   = pd.read_csv(f"/home/unix/cchu/projects/ZmanR/pqe/results/06/ctet_mos/spearman/mcumi_igg_full_ctet_spearman.csv",   index_col=0)
atrem_df = pd.read_csv(f"/home/unix/cchu/projects/ZmanR/pqe/results/06/ctet_mos/spearman/mcumi_atrem_full_ctet_spearman.csv", index_col=0)
genes = list(set(igg_df.index[igg_df["p_value"] < 0.05]) & set(atrem_df.index[atrem_df["p_value"] < 0.05]))
print(f"mcumi full ctet gene intersection: {len(genes)} genes")
...
ax.set_title("Palantir pseudotime (mcumi full_ctet)"); ax.set_xlabel("sc_x"); ax.set_ylabel("sc_y")
